...
# соболезную, если нет возможности свернуть ЭТО, библиотека с фейк юзерагентами долго выдавала их, писать свой генератор
# попросту не хотелось, поэтому всё было решено простым списком с юзерагентами
ua_base = [
...
def link(email, password):
    ...
    # регулярка не такая глупая, просто надо как-то убрать название сайта)))
    res = re.findall(r'https:\/\/accounts..........com\/email\/verify\/.+\n.+\n.+\]', raw_email)
    link = ''.join((res[0].split('=\r\n')))
    final_link = ''.join(link.split('3D'))
    return final_link[:-1]
...
# да, в коде присутствуют странные на первый взгляд задержки, но администрация сайта накатила ещё пару каких-то защит,
# поэтому вручную пришлось подбирать задержки(они не финальные, правильные слипы были настроены на дедике)
if __name__ == '__main__':
    num = int(input("Введите количество акков: "))
    full_mails = getMails()
    mails = full_mails[:num]
    m = 1
    for mail in mails:
        try:
            start = time()
            ...
            s = Session()
            # да, я знаю, голый try except, только никаких ошибок, кроме ошибок подключения нет,
            # их конечно несколько, но решаются они одинаков
            try:
                r = s.get('https://accounts.***********.com/register?__cf_chl_jschl_tk__=bcc70746c9c8c6165b33217a29179811d1279e7b-1617985381-0-AclyNV5mMmqksZqI3uIbjfje9D-gH7bb4I5p5aGkTqd3YsFIIiyjuUJqIyNpA_i7b7Xcji56LlCswzgpTmdpiEOeUKjvyUMiTQuBhfvBMOXm1iPqPSGiHF0-0CjSMWi1PviS6nCh0RU2fyEQRpfRg8PvqvYyOvr1apUjFwwOr3Ywq6s8QJNkFg6dgpFaJVmmEhD8cGseukC0vNiG9phCOEdGQu4D2F71y5Fp578XjB61Y2g8oJCcxDAg9gYc1xu9fjGX6w42DDwXjUZrzMBDDViSwAO9rNKf2AkOlY2KIX7E4QstYzXHLKjnftGbzv-2UO4U1Oy-j7zpalDXytByj-4yukLpbGftQq9W66tcpsLmc3opHufJvebhYbmN-Z10QHVh-rPOnzPQ-3R2ykQ1IJNRmn5QafpcIZB2nNwHxyayVXBNXPJQBTWu0MFyxvHRGA', headers={
                    'User-Agent': u_a
                }, proxies=proxy)
            except:
                print("Ошибка подключения. Сплю 1 секунду.")
                sleep(1)
                r = s.get('https://accounts.***********.com/register?__cf_chl_jschl_tk__=bcc70746c9c8c6165b33217a29179811d1279e7b-1617985381-0-AclyNV5mMmqksZqI3uIbjfje9D-gH7bb4I5p5aGkTqd3YsFIIiyjuUJqIyNpA_i7b7Xcji56LlCswzgpTmdpiEOeUKjvyUMiTQuBhfvBMOXm1iPqPSGiHF0-0CjSMWi1PviS6nCh0RU2fyEQRpfRg8PvqvYyOvr1apUjFwwOr3Ywq6s8QJNkFg6dgpFaJVmmEhD8cGseukC0vNiG9phCOEdGQu4D2F71y5Fp578XjB61Y2g8oJCcxDAg9gYc1xu9fjGX6w42DDwXjUZrzMBDDViSwAO9rNKf2AkOlY2KIX7E4QstYzXHLKjnftGbzv-2UO4U1Oy-j7zpalDXytByj-4yukLpbGftQq9W66tcpsLmc3opHufJvebhYbmN-Z10QHVh-rPOnzPQ-3R2ykQ1IJNRmn5QafpcIZB2nNwHxyayVXBNXPJQBTWu0MFyxvHRGA', headers={
                    'User-Agent': u_a
                }, proxies=proxy)
            ...
            XSRF = utils.dict_from_cookiejar(s.cookies)["XSRF-TOKEN"]
            AWSALB = utils.dict_from_cookiejar(s.cookies)["AWSALB"]
            AWSALBCORS = utils.dict_from_cookiejar(s.cookies)["AWSALBCORS"]
            LAMBDA_TEST_SESSION = utils.dict_from_cookiejar(s.cookies)["***********_session"]
            payload = {'_token': f'{CSRF}',
                       'name': n,
                       'email': email,
                       'password': password,
                       'phone': phone,
            ...
            print('Регаемся..')
            try:
                r = s.post('https://accounts.***********.com/register?fehj=fejfek', proxies=proxy, headers=h, data=payload, files=files)
            ...
            try:
                r = s.get(link(email, password), proxies=proxy,
                ...
                r = s.get(link(email, password), proxies=proxy,
                          headers={'User-Agent': u_a})
            print('Переходим...')
            bearer = utils.dict_from_cookiejar(s.cookies)["accessToken"]
            headers = {
            ...
            try:
                r = s.get("https://accounts.***********.com/api/user",
                          proxies=proxy,
                          headers=headers)
            except:
                print("Ошибка подключения. Сплю 1 секунду.")
            ...
            key = resp['apiToken']
            finish = time()
            print(f'Готово | {round(finish - start, 2)} sec')
            s.close()
            with open('result.txt', 'a') as f:
                f.write(f'{email}:{password}:{username}:{key}\n')
            full_mails.remove(mail)
            with open('mails.txt', 'w') as f:
                for m in full_mails:
                    f.write(m + '\n')
            print('')
            sleep(20)
        # данный except пропускает плохие аккаунты(по принту понятно, но я так, на всякий)
        except KeyError:
            print('Херовый акк, скипаю.')
            full_mails.remove(mail)
            with open('mails.txt', 'w') as f:
                for m in full_mails:
                    f.write(m + '\n')
        # данный except обрабатывает или зарегистрированные аккаунты, или же заблокированные
        except IndexError:
            print('Не пришло письмо.')
            full_mails.remove(mail)
            with open('mails.txt', 'w') as f:
                for m in full_mails:
                    f.write(m + '\n')
    print("Работа окончена, нажмите ENTER...")
    input()
